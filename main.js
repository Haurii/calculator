...
function toClear() {
    return (toggle = !toggle) || !result ? clearAll() : result = clearLast();
}
...
function updateDisplay() {
    ...
    display.textContent = result || "0";  // Tampilkan 0 jika result kosong
}
...
function operations(e) {
    ...
    if (e.target.matches('[data-value]')) updateResult(value);
...
function init(e) {
    ...
    if (e.target.closest(".calculator")) operations(e);  // Diperbaiki
    rules();
    updateDisplay();
}
